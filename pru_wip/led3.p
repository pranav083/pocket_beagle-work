...
// this defines a memory location inside the PRUs address space which we can use to enable
// the PRU to see the BBBs memory as if it were the PRU''s memory (and other things too).
...
L1:
        QBBS      LED_LOW, R31.t14         // if the bit is high, jump to LED_HIGH
        QBBC      LED_HIGH, R31.t14          // if the bit is low, jump to LED_LOW
...
LED_LOW:
        ...
        MOV R3, GPIO_BANK1+GPIO_CLEARDATAOUT// load the address we wish to write to. Note that every
                                            ...
                                            // we do NOT write a 0 to turn it off.
        SBBO R2, R3, 0, 4                   // write the contents of R2 out to the memory address
                                            // contained in R3. Use no offset from that address
                                            // and copy all 4 bytes of R2
